...
module.exports = async function(req, res){
  const { url } = getCfg();
  if (req.method === 'GET'){
    const q = req.query || {}; const rid = q.id; const last_edit = q.last_edit;
    if (rid && last_edit){
      const h = headersJson();
      const r = await fetch(`${url}/rest/v1/last_edits?report_id=eq.${encodeURIComponent(rid)}&select=snapshot,edited_at`, { headers: h });
      if (!r.ok){ res.status(404).json({ error:'no snapshot' }); return; }
      const rows = await r.json(); const out = Array.isArray(rows)&&rows[0]?rows[0]:null;
      res.setHeader('Cache-Control','no-store');
      res.status(200).json(out||{});
      return;
    }
    const category = q.category; const search = q.search; const sort = q.sort || 'recent'; const page = parseInt(q.page||'1',10)||1; const page_size = parseInt(q.page_size||'20',10)||20;
  ...
  if (req.method === 'POST'){
    ...
    const keys=[ 'ai_rating','twitter_handle','ticker','launchpad','labels','basic_info','team_background','dev_activity_rating','dev_activity_note','token_mechanism','progress_plan','business_potential','competitors','focus_points','sources','generated_at','content' ];
